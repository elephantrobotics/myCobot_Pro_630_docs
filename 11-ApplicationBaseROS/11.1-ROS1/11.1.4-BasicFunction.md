...
`mycobot_630` 现已集成了 力控夹爪 MoveIt 部分。
...
<img src =../../resources/11-ApplicationBaseROS/pro630_force_gripper_moveit_1.png
width ="500"  align = "center">

**注意：** 规划组分为机械臂组和夹爪组，即 `arm_group` 和 `gripper_group` 。

选择对应的规划组之后（默认 `arm_group`，可以计划并执行，演示效果：

<video id="my-video" class="video-js" controls preload="auto" width="100%"
poster="" data-setup='{"aspectRatio":"16:9"}'>
  <source src="../../resources/11-ApplicationBaseROS/pro630_force_moveit_1.mp4" type='video/mp4' >
</video>

如果需要让真实的机械臂和Pro力控夹爪同步执行计划，需要再打开一个控制台终端，运行命令：

```bash
# MyCobot Pro 630默认的Socket IP地址为192.168.1.191，端口号为5001，若不一致，可根据实际的IP地址进行修改。
...
<video id="my-video" class="video-js" controls preload="auto" width="100%"
poster="" data-setup='{"aspectRatio":"16:9"}'>
  <source src="../../resources/11-ApplicationBaseROS/pro630_force_moveit_2.mp4" type='video/mp4' >
</video>
